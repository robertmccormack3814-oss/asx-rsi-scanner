...
def estimate_price_for_rsi(daily_hist,now_sydney,target=40.0):
    """Solve today's hypothetical close that makes the same provisional Wilder RSI equal target."""
    close=prior_closes(daily_hist,now_sydney)
    if len(close)<CONFIG["rsi_period"]+1:return None
...
def refresh_dashboard(dashboard,active,completed,new_exits,live_updates):
    ...
    dashboard["generated_at"]=now_iso(); dashboard["completed_trades"]=completed; dashboard["active_signals"]=sorted(active.values(),key=lambda x:x.get("symbol",""))
...
def main():
    ...
    if not is_market_monitor_window(now_sydney):print(f"Outside ASX monitoring window: {now_sydney.isoformat()}");return
    ...
    signals=list(active.values()); crossings=0; checked=0; new_exits=[]; live_updates={}
    for start in range(0,len(signals),BATCH_SIZE):
        ...
        try:
            daily_df=yf.download(tickers,period="3mo",interval="1d",group_by="ticker",auto_adjust=False,threads=True,progress=False); intraday_df=yf.download(tickers,period="1d",interval="5m",group_by="ticker",auto_adjust=False,threads=True,progress=False)
        ...
        for sig in batch:
            ...
            key=signal_key(sig); daily_hist=one_history(daily_df,ticker,multi); intraday_hist=one_history(intraday_df,ticker,multi)
            if daily_hist.empty or intraday_hist.empty:print(f"{sym}: missing daily or intraday data");continue
            latest_price=float(intraday_hist["Close"].iloc[-1]); live_rsi=provisional_daily_rsi(daily_hist,latest_price,now_sydney)
            ...
            latest_state[key]={"symbol":sym,"entry_date":sig.get("entry_date"),"checked_at":now_iso(),"price":latest_price,"provisional_rsi10":live_rsi,"gain_pct":gain_pct,"rsi40_target_price":target,"rsi40_move_pct":target_move}
